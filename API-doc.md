...
```
> `collaboratorIds` solo se rellena en respuestas de una sola playlist (`GET /api/playlists/{id}`, canje de magic link y add/remove de canciones). En los listados (`/my`, `/shared`, `/user/{userId}`) viene `null` para evitar un N+1.
