...
import { Camera, ImageOff, Save, Shield, User as UserIcon } from "lucide-react";
...
/**
 * Página de edición del perfil propio (`/profile`). Es la única forma desde la
 * UI de tocar los campos del {@code Profile} del backend (nombre visible y
 * foto). La password sigue gestionándose desde el panel admin.
 *
 * <p>El nombre se persiste con {@code PUT /api/me/profile}; el avatar viaja
 * por un endpoint multipart aparte ({@code POST /api/me/profile/picture}),
 * mismo patrón que la carátula de playlist. Pulsar "Quitar foto" envía
 * {@code DELETE /api/me/profile/picture} sin tocar el nombre. El componente
 * se queda en {@code use client} y consume {@code useMe()}, así reusamos la
 * caché de React Query para que la UI vuelva a leer el avatar nuevo justo
 * después de subirlo sin recargar la página.
 */
